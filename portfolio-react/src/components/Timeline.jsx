...
const timelineData = [
  {
    ...
    description:
      "Focus on human-computer interaction, advanced UI/UX design, and immersive technologies. CGPA: 16.5/20",
    details: null,
  },
  {
  ...
  },
  {
    ...
    description:
      "Dual-degree program (UFAZ) between Université de Strasbourg and Azerbaijan State Oil and Industry University. CGPA: 14.5/20",
    details: null,
  },
];
...
function TimelineItem({ item, index }) {
  ...
  return (
    <article
      ref={ref}
      className={`grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 relative reveal ${
        isActive ? "active" : ""
      }`}
    >
      {/* Timeline marker */}
      ...
      <div
        ...
        } bg-cream/5 p-8 md:p-10 rounded-lg border border-cream/10 transition-all duration-500 hover:bg-cream/[0.08] hover:border-cream/20 hover:-translate-y-1 ml-12 md:ml-0 md:my-4`}
      >
        ...
        {item.details && (
          <ul className="list-none leading-relaxed font-light opacity-90">
            {item.details.map((detail, i) => (
              <li
                key={i}
                className="mb-3 pl-6 relative before:content-['—'] before:absolute before:left-0 before:opacity-50"
              >
                {detail}
              </li>
            ))}
...
export function Timeline() {
  ...
  return (
    <section
      id="experience"
      className="py-16 px-8 md:px-16 bg-burgundy text-cream relative overflow-hidden"
    >
      <div
        ref={ref}
        className={`mb-20 relative reveal ${isActive ? "active" : ""}`}
      >
